...
export async function GET() {
  try {
    ...
    const hydraIssuerUrl =
      process.env.NEXT_PUBLIC_HYDRA_PUBLIC_URL ?? hydraDirectUrl;
    ...
    try {
      idTokenClaims = await Sentry.startSpan(
        ...
        async (span) => {
          ...
          console.debug(
            '[auth/session] Verifying ID token with JWKS:',
            { jwksUri, issuer: hydraIssuerUrl, decodedToken },
            ' token id:',
            idToken.substring(0, 10) + '...'
          );
          const { payload } = await jwtVerify(idToken, JWKS, {
            issuer: hydraIssuerUrl + '/', // Verify issuer matches the public-facing Hydra URL
      ...
      const isExpired =
        errorMessage.includes('expired') || errorMessage.includes('exp');
    ...
    const expiresAt =
      meta?.expires_at || (idTokenClaims.exp ? idTokenClaims.exp * 1000 : null);
