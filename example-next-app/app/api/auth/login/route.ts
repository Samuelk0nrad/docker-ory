...
export async function GET(req: NextRequest) {
  return Sentry.startSpan({ op: "auth.login", name: "OAuth Login Initiation" }, async (span) => {
    try {
      Sentry.addBreadcrumb({
        category: "oauth",
        message: "Starting OAuth login initiation",
        level: "info",
      });
      ...
      Sentry.addBreadcrumb({
        category: "oauth",
        message: "Generating PKCE code challenge",
        level: "info",
      });
      ...
      span.setAttribute("oauth.has_return_to", !!returnTo);
      span.setAttribute("oauth.pkce_method", "S256");

      Sentry.addBreadcrumb({
        category: "oauth",
        message: "OAuth login initiation completed",
        level: "info",
      });
